...
function makeHealth(id: string): ProviderHealthStatus {
  return { id, healthy: true, consecutiveErrors: 0, lastErrorAt: null, lastSuccessAt: null, unhealthySince: null, activeSessionCount: 0 };
}
...
export class ProviderPool {
  ...
  refreshFromConfig(members: ProviderConfigRow[], balance: Partial<ProviderBalanceConfig> = {}): void {
    this.members = members.map((member) => ({ ...member, weight: Math.max(1, Math.min(100, member.weight || 1)) }));
    this.balance = {
      ...DEFAULT_BALANCE,
      ...balance,
      unhealthyThreshold: Math.max(1, Math.floor(balance.unhealthyThreshold ?? DEFAULT_BALANCE.unhealthyThreshold)),
      recoveryIntervalMs: Math.max(1_000, Math.floor(balance.recoveryIntervalMs ?? DEFAULT_BALANCE.recoveryIntervalMs)),
    };
  ...
  selectProvider(sessionId?: string): ProviderConfigRow | undefined {
    ...
    if (candidate && this.isHealthy(candidate)) {
      const selected = this.members.find((member) => member.id === candidate && member.enabled === 1);
      if (selected) return selected;
    }
    const candidates = this.members.filter((member) => member.enabled === 1 && this.isHealthy(member.id));
    ...
    if (this.balance.strategy === 'failover') {
      ...
      for (const member of candidates) {
        if (offset < member.weight) { selected = member; break; }
  ...
  reportFailure(id: string, immediate = false): void {
    const health = this.getHealth(id);
    health.consecutiveErrors = immediate ? Math.max(this.balance.unhealthyThreshold, health.consecutiveErrors + 1) : health.consecutiveErrors + 1;
    health.lastErrorAt = Date.now();
    if (health.consecutiveErrors >= this.balance.unhealthyThreshold) {
      health.healthy = false;
      health.unhealthySince ??= Date.now();
      for (const [sessionId, providerId] of this.sticky) if (providerId === id) this.sticky.delete(sessionId);
    }
  }

  refreshRecoveryState(now = Date.now()): void {
    for (const health of this.health.values()) {
      if (!health.healthy && health.unhealthySince !== null && now - health.unhealthySince >= this.balance.recoveryIntervalMs) {
        health.healthy = true;
        health.consecutiveErrors = 0;
        health.unhealthySince = null;
      }
...
export function mapProviderToPiProvider(config: ProviderConfigRow, credentials: Record<string, unknown>): PiProviderSelection {
  const env: NodeJS.ProcessEnv = {};
  const apiKey = credentials.apiKey ?? credentials.api_key ?? credentials.token;
  if (typeof apiKey === 'string' && apiKey) env[providerEnvName(config.provider, 'API_KEY')] = apiKey;
  if (config.base_url) env[providerEnvName(config.provider, 'BASE_URL')] = config.base_url;
  const hash = crypto.createHash('sha256').update(JSON.stringify({ id: config.id, provider: config.provider, modelId: config.model_id, baseUrl: config.base_url, credentials })).digest('hex');
  return { provider: config.provider, modelId: config.model_id, env, hash };
}
