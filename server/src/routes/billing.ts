...
import {
  adjustBalance, assignPlan, cancelSubscription, checkBillingAccess, checkQuota, createBillingPlan, createRedeemCode, deleteBillingPlan, deleteQuotaOverride, deleteRedeemCode, getBalance, getBillingAuditLog, getBillingMinStartBalance, getBillingPlan, getDailyUsageHistory, getUserEffectivePlan, isBillingEnabled, listBalanceTransactions, listBillingPlans, listQuotaOverrides, listRedeemCodes, redeemCode, setBillingEnabled, setBillingMinStartBalance, setQuotaOverride, updateBillingPlan,
} from '../billing.js';
import type { QuotaScopeType } from '../billing.js';
...
export function createBillingRoutes(db: Database.Database) {
  ...
  const readQuotaScope = (c: { req: { query: (key: string) => string | undefined } }) => ({ agentId: c.req.query('agentId') ?? null, workspaceJid: c.req.query('workspaceJid') ?? null });
  app.get('/my/quota', (c) => { const user = c.get('user')!; return c.json(checkQuota(db, user.id, user.role, readQuotaScope(c))); });
  app.get('/my/access', (c) => { const user = c.get('user')!; return c.json(checkBillingAccess(db, user.id, user.role, readQuotaScope(c))); });
  ...
  app.get('/admin/quotas', manage, (c) => c.json({ quotas: listQuotaOverrides(db) }));
  app.put('/admin/quotas/:scopeType/:scopeId', manage, async (c) => {
    const scopeType = c.req.param('scopeType');
    if (scopeType !== 'user' && scopeType !== 'agent' && scopeType !== 'workspace') return c.json({ error: '配额作用域类型无效' }, 400);
    const body = await c.req.json().catch(() => ({})) as Record<string, unknown>;
    try {
      const quota = setQuotaOverride(db, {
        scopeType: scopeType as QuotaScopeType,
        scopeId: c.req.param('scopeId'),
        dailyTokenQuota: numberOrNull(body.dailyTokenQuota ?? body.daily_token_quota),
        dailyCostQuota: numberOrNull(body.dailyCostQuota ?? body.daily_cost_quota),
        weeklyTokenQuota: numberOrNull(body.weeklyTokenQuota ?? body.weekly_token_quota),
        weeklyCostQuota: numberOrNull(body.weeklyCostQuota ?? body.weekly_cost_quota),
        monthlyTokenQuota: numberOrNull(body.monthlyTokenQuota ?? body.monthly_token_quota),
        monthlyCostQuota: numberOrNull(body.monthlyCostQuota ?? body.monthly_cost_quota),
      });
      return c.json({ quota });
    } catch (error) { return c.json({ error: error instanceof Error ? error.message : '配额保存失败' }, 400); }
  });
  app.delete('/admin/quotas/:scopeType/:scopeId', manage, (c) => {
    const scopeType = c.req.param('scopeType');
    if (scopeType !== 'user' && scopeType !== 'agent' && scopeType !== 'workspace') return c.json({ error: '配额作用域类型无效' }, 400);
    return deleteQuotaOverride(db, scopeType, c.req.param('scopeId')) ? c.json({ success: true }) : c.json({ error: '配额不存在' }, 404);
  });
