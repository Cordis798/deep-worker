...
function provider(id: string, weight = 1): ProviderConfigRow {
  return { id, owner_user_id: 'u', name: id, provider: id, model_id: `${id}-model`, base_url: null, secret_ref: '', enabled: 1, weight, created_at: '2026-01-01T00:00:00.000Z', updated_at: '2026-01-01T00:00:00.000Z' };
}
...
describe('ProviderPool', () => {
  ...
  it('按权重选择并在失败后切换到健康 Provider', () => {
    const pool = new ProviderPool();
    pool.refreshFromConfig([provider('heavy', 2), provider('light', 1)], { strategy: 'weighted', unhealthyThreshold: 1 });
    expect([pool.selectProvider('a')?.id, pool.selectProvider('b')?.id, pool.selectProvider('c')?.id]).toEqual(['heavy', 'heavy', 'light']);
  ...
  it('按 failover 顺序恢复不健康 Provider', () => {
    ...
    try {
      ...
      const pool = new ProviderPool();
      pool.refreshFromConfig([provider('primary'), provider('backup')], { strategy: 'failover', unhealthyThreshold: 1, recoveryIntervalMs: 60_000 });
  ...
  it('映射 Pi Provider 时只输出允许的凭据环境变量，不暴露密文', () => {
    const mapped = mapProviderToPiProvider({ ...provider('openai'), base_url: 'https://example.test', secret_ref: 'v1.secret' }, { apiKey: 'secret-value', ignored: 'not-exported' });
    expect(mapped).toMatchObject({ provider: 'openai', modelId: 'openai-model' });
    expect(mapped.env).toEqual({ OPENAI_API_KEY: 'secret-value', OPENAI_BASE_URL: 'https://example.test' });
