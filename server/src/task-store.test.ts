...
describe('任务状态存储', () => {
  it('限制固定间隔并计算五字段 Cron', () => {
    ...
    expect(computeNextRunAt('cron', '*/5 * * * *', new Date('2026-08-21T10:01:00.000Z'))).toBe(
  ...
  it('重启时周期任务记为 missed，一次性任务保留补跑记录', () => {
    ...
    const futureOnce = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString();
    ...
    const once = createTask(db, {
      ...
      executionType: 'agent',
      scheduleType: 'once',
      scheduleValue: futureOnce,
      prompt: '一次',
    });
    db.prepare('UPDATE scheduled_tasks SET next_run_at = ? WHERE id IN (?, ?)').run(
      old.toISOString(),
      periodic.id,
      once.id,
    );
    const runs = materializeDueTasks(
      db,
  ...
  it('租约过期后重新排队，达到上限后失败', () => {
    ...
    db.prepare(
      "UPDATE task_runs SET status = 'running', attempt = 1, lease_owner = 'dead', lease_expires_at = ? WHERE id = ?",
    ...
    db.prepare(
      "UPDATE task_runs SET status = 'running', attempt = 3, lease_owner = 'dead', lease_expires_at = ? WHERE id = ?",
    ).run('2020-01-01T00:00:00.000Z', run.id);
