...
const { post, openTurnStream } = vi.hoisted(() => {
  const post = vi.fn();
  const openTurnStream = vi.fn((_workspace: string, _session: string, _turn: string, handlers: { onEvent: (event: unknown) => void }) => {
    queueMicrotask(() => {
      handlers.onEvent({ eventType: 'init', statusText: 'agent started' });
      handlers.onEvent({ eventType: 'tool_use_start', toolName: 'bash', toolUseId: 'tool-1' });
...
describe('聊天流状态', () => {
  it('聚合文本、工具轨迹并在终态结束生成', async () => {
    ...
    expect(messages[1]?.events.map((event) => event.eventType)).toEqual(['init', 'tool_use_start', 'text_delta', 'status']);
