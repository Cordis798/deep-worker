  ...
  return (
    <AuthPage title="欢迎回来" subtitle="登录 Deep Worker，继续你的工作。">
      <form onSubmit={submit} className="space-y-4">
        ...
        <button disabled={submitting} className="w-full rounded-xl bg-slate-950 px-4 py-3 text-sm font-semibold text-white transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-50">
          {submitting ? '登录中…' : '登录'}
        </button>
      </form>
      <p className="mt-6 text-center text-sm text-slate-500">还没有账号？ <Link className="font-semibold text-indigo-600 hover:text-indigo-800" to="/register">注册</Link></p>
    </AuthPage>
  );
}
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[radial-gradient(circle_at_top_left,#e0e7ff,transparent_40%),#f8fafc] p-5">
      <section className="w-full max-w-md rounded-[2rem] border border-white/80 bg-white/90 p-7 shadow-xl shadow-indigo-950/10 backdrop-blur sm:p-9">
        <div className="mb-8 flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-slate-950 font-semibold text-white">DW</div><div className="font-semibold">Deep Worker</div></div>
...
function Field({ label, value, onChange, type = 'text', autoComplete }: { label: string; value: string; onChange: (value: string) => void; type?: string; autoComplete?: string }) {
  return <label className="block text-sm font-medium text-slate-700"><span>{label}</span><input required type={type} autoComplete={autoComplete} value={value} onChange={(event) => onChange(event.target.value)} className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-3.5 py-3 outline-none transition placeholder:text-slate-300 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10" /></label>;
}
