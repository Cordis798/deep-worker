  ...
  return (
    <div className="grid min-h-full grid-cols-1 lg:grid-cols-[280px_minmax(0,1fr)]">
      ...
      <section className="flex min-h-[calc(100vh-4rem)] min-w-0 flex-col">
        ...
        <form onSubmit={submitMessage} data-mobile-chat="composer" className="border-t border-slate-200 bg-white p-3 sm:p-5"><div className="mx-auto flex max-w-4xl items-end gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-2 shadow-sm focus-within:border-indigo-400 focus-within:ring-4 focus-within:ring-indigo-500/10"><textarea data-mobile-chat="input" value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void submitMessage(event); } }} disabled={!canSend} rows={2} placeholder={canSend ? '输入消息，Enter 发送，Shift+Enter 换行' : '请先选择一个可用会话'} className="min-h-12 min-w-0 flex-1 resize-none bg-transparent px-2 py-2 text-sm outline-none disabled:cursor-not-allowed" /><button data-mobile-chat="send" disabled={!canSend || !draft.trim()} className="rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-40">发送</button></div></form>
      </section>
    </div>
  );
}
