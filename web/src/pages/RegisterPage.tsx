...
export function RegisterPage() {
  ...
  useEffect(() => {
    void api
      .get<{ requireInviteCode: boolean }>('/api/auth/register/status')
      .then((status) => setRequireInviteCode(status.requireInviteCode))
  ...
  return (
    <AuthPage title="创建账号" subtitle="建立一个新的工作台账号。">
      <form onSubmit={submit} className="space-y-4">
        ...
        />
        <Field
          label={`邀请码${requireInviteCode ? '' : '（可选）'}`}
          value={inviteCode}
        ...
        <button
          disabled={submitting}
          className="w-full rounded-xl bg-slate-950 px-4 py-3 text-sm font-semibold text-white transition hover:bg-indigo-700 disabled:opacity-50"
        >
          {submitting ? '注册中…' : '注册'}
        </button>
      </form>
