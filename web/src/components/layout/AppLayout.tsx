...
const navItems = [
  ...
  { to: '/setup/providers', label: 'Provider', icon: '◉' },
  { to: '/setup/channels', label: '渠道', icon: '↗' },
];
  ...
  return (
    <aside className="flex h-full w-64 shrink-0 flex-col border-r border-slate-200 bg-white px-4 py-5">
      <div className="mb-8 flex items-center gap-3 px-2">
        <img className="h-10 w-10 rounded-2xl" src="/icon.svg" alt="Deep Worker 标志" />
        <div>
...
export function AppLayout() {
  ...
  const location = useLocation();
  const title =
    navItems.concat(adminItems).find((item) => location.pathname.startsWith(item.to))?.label ??
    '工作台';
  ...
  return (
    <div className="min-h-screen bg-[#f6f7fb] text-slate-950">
      <div className="flex min-h-screen">
        ...
        <div
          className={`fixed inset-y-0 left-0 z-40 transition-transform md:hidden ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}
        >
