...
| Agent Router | 根据任务意图、岗位标签和能力选择 Agent，持久化 Plan/Task/Event，按依赖顺序执行并汇总结果 | `server/src/agent-router/` |
| 并发安全 | Plan 与 Task 使用 SQLite 租约；重复 dispatch 返回冲突，旧 worker 不能覆盖新租约结果 | `server/src/agent-router/store.ts` |
| Web/IM | Web 可创建和调度计划；IM 支持 `/route` 与 `/single`，两者进入同一执行链 | `web/src/pages/ChatPage.tsx`、`server/src/im/` |
...
3. 在 Router 上补充审批、预算、取消、并行只读任务和 uncertain 状态；保持现有单 Agent 链路兼容。
